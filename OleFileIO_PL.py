...
def i32(c, o=0):
    ...
    o: offset of bytes to convert in string
    """
    return int(ord(c[o]) + (ord(c[o + 1]) << 8) +
...
class OleFileIO:
    ...
    def open(self, filename):
        ...
        if hasattr(filename, 'read'):
            ...
            # TODO: if larger than 1024 bytes, this could be the actual data =>
            # StringIO
        ...
        # TODO: fix code to fail with clear exception when filesize cannot be
        # obtained
...
# #              raise IOError, 'incorrect DIFAT'
    ...
    def loadminifat(self):
        ...
        #    32 bits indexes:
        nb_minisectors = (
            self.root.size + self.MiniSectorSize - 1) / self.MiniSectorSize
        used_size = nb_minisectors * 4
        debug(
            'loadminifat(): minifatsect=%d, nb FAT sectors=%d, used_size=%d, \
        ...
        # Then shrink the array to used size, to avoid indexes out of
        # MiniStream:
        debug(
            'MiniFAT shrunk from %d to %d sectors' %
            (len(
                self.minifat),
                nb_minisectors))
...
# #            self.direntries.append(_OleDirectoryEntry(entry, sid, self))
    ...
    def _load_direntry(self, sid):
        ...
        if sid < 0 or sid >= len(self.direntries):
            self._raise_defect(
                DEFECT_FATAL,
                "OLE directory index out of range")
    ...
    def _list(self, files, prefix, node, depth=None):
        """
        (listdir helper)
        ...
        for entry in node.kids:
            if (depth is None or depth > 0) and entry.kids:
                self._list(files, prefix, entry, newDepth)
            else:
                files.append(prefix[1:] + [entry.name])

    def listdir(self, depth=None):
        """
        Return a list of streams stored in this file
        """
        files = []
        self._list(files, [], self.root, depth)
        return files
    ...
    def getproperties(self, filename, convert_time=False, no_conversion=None):
        ...
        filename: path of stream in storage tree (see openstream for syntax)
        convert_time: bool, if True timestamps will be converted to Python
        datetime no_conversion:
        None or list of int, timestamps not to be converted
        ...
        for i in range(num_props):
            try:
                ...
                if type == VT_I2:  # 16-bit signed integer
                    ...
                    # CodePageString, see
                    # http://msdn.microsoft.com/en-us/library/dd942354.aspx
                    # size is a 32 bits integer, including the null terminator,
                    # and possibly trailing or embedded null chars
                    # TODO: if codepage is unicode, the string should be
                    # converted as such
                    ...
                    # "the string should NOT contain embedded or
                    # additional trailing null characters."
                    count = i32(s, offset + 4)
                    value = _unicode(s[offset + 8:offset + 8 + count * 2])
                elif type == VT_FILETIME:
                    value = long(i32(s, offset + 4)) + \
                        (long(i32(s, offset + 8)) << 32)
                    # FILETIME is a 64-bit int: "number of 100ns periods
                    # since Jan 1,1601".
                    if convert_time and id not in no_conversion:
                        debug(
                            'Converting property #%d to python datetime, \
                            value=%d=%fs' %
                            (id, value, float(value) / 10000000))
                        # convert FILETIME to Python datetime.datetime
                        # inspired from
                        # http://code.activestate.com/recipes/
                        # 511425-filetime-to-datetime/
                        _FILETIME_null_date = datetime.datetime(
                            1601,
                            1,
                            1,
                            0,
                        ...
                        value = _FILETIME_null_date + \
                            datetime.timedelta(microseconds=value / 10)
                    else:
                        # legacy code kept for backward compatibility: returns
                        # a number of seconds since Jan 1,1601
                    ...
                    debug(
                        'property id=%d: type=%d not implemented \
                        in parser yet' %
                        (id, type))
                ...
                # VT_VECTOR is a 32 uint giving the number of items, followed
                # by the items in sequence.
                # The VT_VECTOR value is combined with
                # the type of items, e.g. VT_VECTOR|VT_BSTR
...
"""
    ...
    for filename in sys.argv[1:]:
            ...
            if streamname[-1][0] == "\005":
                ...
                for k, v in props:
                    ...
                    if isinstance(v, basestring):
                        ...
                        for c in (1, 2, 3, 4, 5, 6, 7, 11, 12, 14, 15, 16,
        ...
        if ole.exists('worddocument'):
            ...
            print ole.get_type('worddocument')
